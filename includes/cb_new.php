	...
	?>
		<div align="center" width="95%" class="memberbg_6">
			...
			<div width="100%" style="padding:1%;" class="memberbg_4">
				<?php
				if (isset($_GET['admin']) AND $_SESSION['rank'] > 5)
				{
					if ($_GET['admin'] == "see_del")
					{
					 	
						$cb_select = $db->prepare('SELECT * FROM cb_test WHERE salon = "" AND to_id = ? OR
					 	 salon = "" AND to_id = 0 ORDER BY post_date DESC LIMIT 20');
					 	$cb_select->execute(array($_SESSION['id']));	
					}
					elseif ($_GET['admin'] == "see_whisp")
					{
						$cb_select = $db->query('SELECT * FROM cb_test WHERE del = 0 AND salon = ""  ORDER BY post_date DESC LIMIT 20');
					}
				}
				else
				{	
					$cb_select = $db->prepare('SELECT * FROM cb_test WHERE del = 0 AND salon = "" AND to_id = ? OR
					 del = 0 AND salon = "" AND to_id = 0  ORDER BY post_date DESC LIMIT 20');
					 $cb_select->execute(array($_SESSION['id']));	
				}
				 ...
				 while ($line = $cb_select->fetch())
				 {
					...
					$name_cb = $select_member->fetch();
					 ...
					 if ($line['del'] == 0)	
					 {
					 	...
					 	if ($_SESSION['rank'] >= $name_cb['rank'])
					 	{
					 		$delbutton = '<a href="index?p=cb&del='. $line['id'].'" style="color:red;">[x]</a> ';
					 	}
						?>
						<p style="text-align:left;"> <?= $delbutton ?>
