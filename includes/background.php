  ...
  if ($_SESSION['connected'])
  {
    if ($_SESSION['rank'] > 4)
    {
      if (isset($_GET['type']))
      {
        ...
        ?>
              <tr>
                <td class="bgtd" width="80px"><img src="pics/ico/bg_sub_<?= $line['id']?>" width="100%" alt="" /></td>
      ...
      elseif (isset($_GET['sub']))
      {
        ...
        <?php
        $select = $db->prepare('SELECT * FROM bg_id WHERE sub_id = ? AND level <= ? ORDER BY title ASC'); $select->execute(array($sub, $_SESSION['rank']));
        while ($line = $select->fetch())
        {
          switch ($line['level']) 
          {
        ...
        ?>
              <tr>
                <td class="bgtd" width="80px"><img src="pics/ico/bg_id_<?= $line['id']?>" width="100%" alt="" /></td>
      ...
      else
      {
       ...
       ?>
        ...
        while ($line = $select->fetch())
        {
          ...
          ?>
              <tr>
                <td class="bgtd" width="80px"><img src="pics/ico/bg_type_<?= $line['id']?>" width="100%" alt="" /></td>
