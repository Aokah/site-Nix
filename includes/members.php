<?php function members ()
{
	...
	$total = $db->exec('SELECT COUNT(*) AS cons FROM rank > 0 AND pnj = 1'); $total = $total->fetch();
	$op = $db->exec('SELECT COUNT(*) AS op FROM rank = 7'); $op = $op->fetch();
	$mj = $db->exec('SELECT COUNT(*) AS mj FROM rank = 6'); $mj = $mj->fetch();
	$modo = $db->exec('SELECT COUNT(*) AS dieu FROM rank = 5'); $modo = $modo->fetch();
	$enca = $db->exec('SELECT COUNT(*) AS op FROM rank = 4'); $enca = $enca->fetch();
	$jplus = $db->exec('SELECT COUNT(*) AS mj FROM rank = 3'); $jplus = $jplus->fetch();
	$joueur = $db->exec('SELECT COUNT(*) AS dieu FROM rank = 2'); $joueur = $joueur->fetch();
	$new = $db->exec('SELECT COUNT(*) AS op FROM rank = 1'); $new = $new->fetch();
