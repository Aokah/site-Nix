...
if ($_SESSION['connected'])
{
	...
	if (isset($_GET['viewavis']))
	{
		$perso = intval($_GET['viewavis']);
			...
			<tbody>
				...
				<tr>
					<td>
						<table width="640px" cellspacing="15" background="/pics/ico/magiepapercenter.png" style="padding:5%; text-align:center;">
							<tbody>
								<tr>
									<th>Titre</th>
									<th>Nom</th>
	...
	elseif (isset($_GET['perso']))
	{
		...
		if (isset($_GET['modif']))
		{
			...
			if ($line = $page->fetch()) {
				if($_GET['modif'] == "save")
					{
						if(isset($_POST['confirm']))
						{
							...
							switch ($_POST['race']) {
								...
								case 11: $race = "Titanoïde"; break; case 12: $race = "Zaknafein"; break; case 13: $race = "Dragon"; break;}
						...
						elseif(isset($_POST['valid']))
						{
							if ($_SESSION['rank'] >= 5) {
								switch ($_POST['spe_1']) {
									case 0: $spe_1 = $line['specialisation']; $element = 20; break;
								...
								switch ($_POST['spe_2']) {
									case 0: $spe_2 = $line['spe_2'];  $element_2 = 20 ; break;
									...
									if ($element != $origine AND $element != 0 AND $element != 20)
									{
										...
										if ($line = $preverif->fetch())
										{
											...
											if ($line['element'] == $element)
											{
												...
												$update3 = $db->prepare('UPDATE members SET magie_rank = ? WHERE id = ?'); $update3->execute(array($line['rank'],$perso));
											}
											else
											{
				...
				?>
					...
					<p class="name6">ATTENTION : Ne pas effectuer d'échange Spécialisation Primaire - Secondaire ou d'entrer une spécialitation identique à l'autre, (mettre Energie en Spécialitation primaire alors qu'il y a déjà l'Energie en Secondaire par exemple) au prix de provoquer un bug !</p>
					<form action="index?p=perso&perso=<?php echo $perso;?>&modif=save" method="POST">
						<table cellspacing="5" cellpadding="5" class="pnjtable" width="100%">
							<tbody>
								<tr>
									...
									<td>
										<label for="spe_1">Spécialisation primaire :</label>
											<select name="spe_1" type="text">
												...
												<option value="16">Chaleur</option>
												<option value="3">Chaos</option>
												<option value="4">Eau</option>
												<option value="18">Espace</option>
			...
			elseif($_GET['action'] == 'avert')
			{
				if ($_SESSION['rank'] >= 5) {
					...
					else
					{
						...
						<form action="index?p=perso&perso=<?=$perso?>&action=avert" method="POST">
						<table align="center">
							<tbody>
								<th>
									<p>
		...
		if ($line['rank'] == 9) { $grade = "titan";} if ($line['rank'] == 10) { $grade = "crea"; } if ($line['rank'] == 11) { $grade = "principe"; }
		...
		<tbody>
			<tr>
						...
						<tbody>
							...
							<tr>
								<td colspan="4" style="border: 0px grey solid; background-color: grey; color: grey; text-align:justify;">
									...
									<?php } }
									 if ($_SESSION['rank'] >= 5) { if ($line['magieok'] == 0) {?>
				...
				<?php if ($_SESSION['rank'] < 6 ) { $span = "2"; } else { $span = "3"; }
					$select0 = $db->prepare('SELECT COUNT(*) AS plus FROM hrpavis WHERE target_id = ? AND avis = 1 AND sender_rank <= 4');
					$select0->execute(array($perso)); $line0 = $select0->fetch();
					$select1 = $db->prepare('SELECT COUNT(*) AS plusstaff FROM hrpavis WHERE target_id = ? AND avis = 1 AND sender_rank > 4');
					$select1->execute(array($perso)); $line1 = $select1->fetch();
					$select2 = $db->prepare('SELECT COUNT(*) AS moins FROM hrpavis WHERE target_id = ? AND avis = 0 AND sender_rank <= 4');
					$select2->execute(array($perso)); $line2 = $select2->fetch();
					...
					if ($hrpavis >= 10 AND $hrpavis < 20) { $coloravis = 10;} elseif ($hrpavis >= 20 AND $hrpavis < 30) { $coloravis = 20;} elseif ($hrpavis >= 30) { $coloravis = 30;} elseif ($hrpavis < 0) { $coloravis = "negative"; } else { $coloravis = 0; }
					?>
					...
					<table width="640px" cellspacing="5" cellpading="5" class="pnjtable" align="center">
						<tbody>
							...
							<tr>
								...
								<td colspan="2">
									Qualité de jeu : <a class="avis<?= $coloravis?>" href="index?p=perso&viewavis=<?php echo $perso;?>">[<?php echo $hrpavis;?>]</a> <?php 
									if ($_SESSION['rank'] > 2 AND $_SESSION['id'] != $perso) { ?><a href="index?p=perso&perso=<?php echo $perso;?>&action=avisok" class="ok">[+1]</a> <a href="index?p=perso&perso=<?php echo $perso;?>&action=avisko" class="ko">[-1]</a> 
									<?php } ?>
								</td>
							</tr>
							<tr>
