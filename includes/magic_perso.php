<?php function magic_level ()
{
	...
	$presel = $db->prepare('SELECT * FROM members WHERE id = ?'); $presel->execute(array($id)); $presel = $presel->fetch();
		...
		while ($line = $select->fetch())
		{
			switch ($line['element'])
			{
				default : $ico = "Inconnue"; $lore = "Acune aura magique"; break; case 1: $ico = "Air"; $lore  =" en Aeormancie"; break;
		...
		<p>
					...
					{
					?>
						<img src="pics/magie_rank_<?= $line['rank']?>.gif" alt="" /> <img src="pics/magie/Magie_<?= $ico?>.png" alt="" class="magie_type" width="25px" /> <?= $magie; ?> <?= $lore; ?>
					<?php
					}
					else
					{
