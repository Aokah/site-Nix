...
<?php if (isset($_GET['s']))
	{
		...
		$answer = $db->prepare('SELECT s.id AS s_id, s.sender_id AS sender, s.text, s.rank AS level, s.title AS titre, m.id AS id, m.name, m.title, m.rank AS rank, m.technician, m.pionier
		FROM sondage s
